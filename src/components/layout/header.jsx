    ...
    fetchUserInfo((user) => {
      ...
      socket.on("create", ({ data }) => {
        if (user && user.role == "ADMIN") {
          setDataNotif(
            data
              .filter((i) => i.title == "A new leave request")
              .sort(
                (a, b) =>
          ...
          setNumNotif(
            data.filter(
              (i) => i.title == "A new leave request" && i.status == "0"
            ).length
          );
        }
      });
      socket.on("handle", ({ data }) => {
        if (user && user.role != "ADMIN" && user.id == data.userId) {
          const list = data.list.filter(
            (i) => i.staffId == user.id && i.title != "A new leave request"
          );
          setDataNotif(
            list.sort(
              (a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()
    ...
    <div className="header-container sticky top-0 z-30 w-full p-5 bg-white sm:px-7 shadow-lg">
      <div className="flex items-center justify-between max-w-full">
        ...
        {status === 1 && (
                                  ...
                                  <div class="row">
                                    <div class="col-xs-12 col-sm-6 col-sm-offset-3">
                                      <div class="new-message-box">
                                        <div class={icon2}>
                                          <div class={icon} title={titleIcon}>
                                            <i></i>
                                          </div>
                                          <div class={title}>
                                            <p>
                ...
                <li>
                  <div className="relative">
                    {user?.id == undefined ? (
                      <button
                        type="button"
                        className="items-center text-white bg-blue-900 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-lg w-full sm:w-auto px-5 pt-3 pb-2.5 inline-flex text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          ...
          "
                      >
                        <div className="flex flex-col cursor-pointer gap-4 py-4">
                          <>
                            {/* <MenuItem
                              label="ThÔng tin"
                              // onClick={() =>
